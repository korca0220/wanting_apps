...
void main() {
  testWidgets('WdsIconButton fires onPressed', (tester) async {
    var hits = 0;
    await tester.pumpWidget(_wrap(WdsIconButton(
      onPressed: () => hits++,
      semanticLabel: 'Add',
  ...
  testWidgets('WdsDivider renders', (tester) async {
    await tester.pumpWidget(_wrap(const SizedBox(width: 100, child: WdsDivider())));
  ...
  testWidgets('WdsLabel adds asterisk when required', (tester) async {
    await tester.pumpWidget(_wrap(const WdsLabel(text: 'Email', required: true)));
  ...
  testWidgets('WdsSpinner renders a CircularProgressIndicator',
      (tester) async {
