...
void main() {
  ...
  testWidgets('WdsTextField clearable shows reset and clears value', (
    ...
    await tester.pumpWidget(
      _wrap(WdsTextField(controller: ctrl, clearable: true)),
    );
  ...
  testWidgets('WdsTextarea grows to multi-line', (tester) async {
    await tester.pumpWidget(
      _wrap(const SizedBox(width: 300, child: WdsTextarea(minLines: 3))),
    );
  ...
  testWidgets('WdsTooltip renders the wrapped child', (tester) async {
    await tester.pumpWidget(
      _wrap(const WdsTooltip(message: 'Hint', child: Icon(Icons.info))),
    );
        ...
        const SizedBox(
          ...
          child: WdsCard(
            child: WdsCardContent(
              children: [WdsCardTitle('Title'), WdsCardCaption('Caption text')],
            ),
          ),
        ),
      ),
