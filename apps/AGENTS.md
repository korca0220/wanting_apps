...
`lib/` 내부 구조·상태관리·라우팅은 앱별 자유지만, 아래 규칙은 모든 앱이 공통으로 따른다.
...
같은 모듈에서 **2번 이상 호출되지 않는** private 함수·헬퍼는 만들지 않는다. 재사용이 없으면 인라인이 거의 항상 더 짧고 — 호출자가 정의로 점프할 필요도 없다.

예외 (둘 다 충족할 때만):
1. **이름이 도메인 의도를 가리키고 본문은 기계적**일 때 — 이름이 라벨로 가치를 더하는 경우.
2. **본문이 길어** 인라인하면 호출지의 흐름을 끊을 때 (대략 10줄 이상).

`switch` 한 덩어리, `padLeft` 몇 번, `trim()` 한 줄짜리 등은 예외에 해당하지 않는다 — 인라인이 더 읽기 쉽다. enum ↔ string 매핑은 Dart 내장 `enum.name` / `Enum.values.byName(...)` (또는 `firstWhere` + `orElse`)로 헬퍼 자체가 사라지는 경우가 많다.

진짜 재사용이 늘어나는 시점에 **그때** 추출한다.
